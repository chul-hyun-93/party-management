  ...
  try {
    ...
    switch (intent) {
      ...
      case 'create':
        ...
        break;
      default:
        result = { success: false, message: '파티 관련 요청을 인식하지 못했습니다.' };
    }
...
function joinParty(nickname, partyName) {
  ...
  if (!parties[partyName]) {
    return { success: false, message: `"${partyName}" 파티가 존재하지 않습니다. 먼저 파티를 생성해주세요.` };
  }

  // 이미 참가 중인지 확인
  if (parties[partyName].includes(nickname)) {
    return { success: false, message: `"${nickname}"님은 이미 "${partyName}" 파티에 참가 중입니다.` };
  }

  // 파티에 참가
  parties[partyName].push(nickname);
  return { success: true, message: `"${nickname}"님이 "${partyName}" 파티에 참가했습니다!` };
}
...
function leaveParty(nickname, partyName) {
  // 파티가 존재하는지 확인
  if (!parties[partyName]) {
    return { success: false, message: `"${partyName}" 파티가 존재하지 않습니다.` };
  }

  // 참가 중인지 확인
  if (!parties[partyName].includes(nickname)) {
    return { success: false, message: `"${nickname}"님은 "${partyName}" 파티에 참가하고 있지 않습니다.` };
  }

  // 파티에서 탈퇴
  parties[partyName] = parties[partyName].filter(member => member !== nickname);
  return { success: true, message: `"${nickname}"님이 "${partyName}" 파티에서 탈퇴했습니다.` };
}
...
function createParty(nickname, partyName) {
  // 이미 존재하는 파티인지 확인
  if (parties[partyName]) {
    return { success: false, message: `"${partyName}" 파티는 이미 존재합니다.` };
  }

  // 새 파티 생성 및 생성자 자동 참가
  parties[partyName] = [nickname];
  return { success: true, message: `"${nickname}"님이 "${partyName}" 파티를 생성하고 참가했습니다!` };
}